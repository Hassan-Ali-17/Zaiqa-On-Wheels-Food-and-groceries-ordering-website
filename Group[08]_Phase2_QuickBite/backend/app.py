...
import sqlite3, hashlib, os, datetime, base64, uuid, secrets, functools
import jwt as pyjwt

app = Flask(__name__)
app.secret_key = os.environ.get('FLASK_SECRET', 'qb_flask_secret_2024_dev')
JWT_SECRET  = os.environ.get('JWT_SECRET',  'qb_jwt_secret_2024_dev_change_in_production')
JWT_EXPIRY_HOURS = 24
CORS(app)
...
# ── Password hashing — PBKDF2-HMAC-SHA256 (NIST SP 800-132, 310,000 iterations) ──
def hash_password(pw):
    """Hash a password using PBKDF2-HMAC-SHA256 with a random 128-bit salt.
    Format: pbkdf2:sha256:<iterations>$<salt_hex>$<dk_hex>
    Legacy SHA-256 hashes (no $ separator) still work via check_password.
    """
    salt = secrets.token_hex(16)          # 128-bit cryptographically random salt
    iterations = 310_000                  # NIST SP 800-132 recommended minimum for 2023
...
def check_password(pw, stored):
    """Constant-time password check supporting both PBKDF2 and legacy SHA-256."""
    if stored and '$' in stored:          # PBKDF2 format
        try:
            _, algo, rest = stored.split(':', 2)
            iters_str, salt, stored_dk = rest.split('$')
            dk = hashlib.pbkdf2_hmac('sha256', pw.encode('utf-8'), salt.encode('utf-8'), int(iters_str))
            return secrets.compare_digest(dk.hex(), stored_dk)
        except Exception:
            return False
    else:                                 # Legacy plain SHA-256 (seed accounts)
        return secrets.compare_digest(hashlib.sha256(pw.encode()).hexdigest(), stored or '')

# ── JWT helpers ──────────────────────────────────────────────────────────────
def issue_jwt(user_id, role):
    """Issue a signed JWT valid for JWT_EXPIRY_HOURS hours."""
...
def decode_jwt(token):
    """Decode and verify a JWT. Returns (user_id, role) or (None, None)."""
    try:
...
def get_current_user():
    """Extract and verify JWT from Authorization header. Returns (uid, role) or (None, None)."""
    auth = request.headers.get('Authorization', '')
    if auth.startswith('Bearer '):
        return decode_jwt(auth[7:])
    # Fallback: also accept token in X-Auth-Token header (for legacy clients)
...
def require_auth(*allowed_roles):
    """Decorator factory enforcing JWT authentication and optional role-based access control.

    Usage:
        @app.route('/api/admin/stats')
        @require_auth('admin')
        def admin_stats(_auth_uid, _auth_role, **kwargs): ...

        @app.route('/api/orders', methods=['POST'])
        @require_auth('customer')          # only customers may place orders
        def place_order(_auth_uid, _auth_role, **kwargs): ...
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            if uid is None:
                log_audit(None, 'AUTH_FAIL', f'Unauthenticated request to {request.path}')
                return jsonify({'error': 'Authentication required', 'code': 'UNAUTHORIZED'}), 401
            if allowed_roles and role not in allowed_roles:
                log_audit(uid, 'ACCESS_DENIED', f'Role {role} tried {request.path} (requires {allowed_roles})')
                return jsonify({'error': 'Access denied', 'code': 'FORBIDDEN',
...
require_any      = require_auth()           # authenticated, any role
...
def signup():
    ...
    if data['role'] not in ('customer', 'rider', 'manager', 'admin'):
    ...
    try:
        ...
        return jsonify({'success': True, 'user_id': user_id, 'role': data['role'], 'token': token})
    except sqlite3.IntegrityError:
        db.execute('ROLLBACK')
        log_audit(None, 'SIGNUP_FAIL', f"Duplicate email: {data.get('email','?')}")
...
def login():
    ...
    if not user or not check_password(data['password'], user['password_hash']):
        db.close()
        log_audit(None, 'LOGIN_FAIL', f"Failed login for: {data.get('email','?')}")
    ...
    db.close()
    log_audit(user['id'], 'LOGIN', f"{user['role']} logged in from {request.remote_addr}")
...
def logout():
    uid, role = get_current_user()
    if uid:
        log_audit(uid, 'LOGOUT', 'Token invalidated client-side')
...
def get_restaurants():
    db = get_db()
    rows = db.execute("SELECT * FROM restaurants WHERE is_open=1 AND status='approved' ORDER BY rating DESC").fetchall()
    db.close()
...
def get_restaurant(rid):
    ...
    for cat in cats:
        items = db.execute('SELECT * FROM menu_items WHERE category_id=? AND is_available=1', (cat['id'],)).fetchall()
        result['categories'].append({'id':cat['id'],'name':cat['name'],'items':[dict(i) for i in items]})
    # Also include items with no category or orphaned category_id so new items always show
    if cat_ids:
        placeholders = ','.join('?' * len(cat_ids))
        uncategorized = db.execute(
            f'SELECT * FROM menu_items WHERE restaurant_id=? AND is_available=1 AND (category_id IS NULL OR category_id NOT IN ({placeholders}))',
            [rid] + cat_ids
        ).fetchall()
    else:
        uncategorized = db.execute(
            'SELECT * FROM menu_items WHERE restaurant_id=? AND is_available=1', (rid,)
        ).fetchall()
    if uncategorized:
        result['categories'].append({'id': None, 'name': 'Menu', 'items': [dict(i) for i in uncategorized]})
    db.close()
    return jsonify(result)
...
def manager_update_item(iid):
    data = request.json
    db = get_db()
    # Toggle-only update (availability toggle button) — don't overwrite name/price
    if data.get('_toggle'):
        db.execute('UPDATE menu_items SET is_available=? WHERE id=?', (data.get('is_available', 1), iid))
        db.commit()
        db.close()
        return jsonify({'success': True})
    img = save_image(data.get('image'), 'item')
    if img:
        db.execute('UPDATE menu_items SET name=?,description=?,price=?,image_filename=?,is_available=? WHERE id=?',
                   (data['name'],data.get('description'),data['price'],img,data.get('is_available',1),iid))
    else:
        db.execute('UPDATE menu_items SET name=?,description=?,price=?,is_available=? WHERE id=?',
                   (data['name'],data.get('description'),data['price'],data.get('is_available',1),iid))
    db.commit()
    db.close()
    return jsonify({'success': True})
...
def manager_delete_item(iid):
    db = get_db()
    db.execute('UPDATE menu_items SET is_available=0 WHERE id=?',(iid,))
    db.commit()
    db.close()
    return jsonify({'success': True})
...
def place_order():
    """Place a restaurant order — fully atomic via BEGIN/COMMIT/ROLLBACK.

    Transaction scenario 1 (Phase 2 requirement):
    Steps: INSERT order → INSERT order_items (each validated) → validate+UPDATE promo →
           INSERT OR IGNORE loyalty_points → UPDATE loyalty + INSERT loyalty_transaction →
           UPDATE promo used_count → COMMIT
    Rollback conditions: invalid menu item, inactive/exhausted promo, loyalty underflow,
                         any DB constraint violation, or any unexpected exception.
    """
    data = request.json
    if not data:
    ...
    try:
        ...
        cur = db.execute(
            ...
            ' VALUES (?,?,?,?,?,?,?,?,?,?)',
            (cid, rid, total, delivery_fee, platform_fee, tip,
        ...
        # Step 2: INSERT order items — validate each belongs to correct restaurant
        for item in data.get('items', []):
            mi = db.execute(
                'SELECT id FROM menu_items WHERE id=? AND restaurant_id=? AND is_available=1',
                (item['item_id'], rid)
            ).fetchone()
            if not mi:
                raise ValueError(f"Menu item {item['item_id']} not available at restaurant {rid}")
            db.execute(
                'INSERT INTO order_items (order_id,item_id,quantity,unit_price) VALUES (?,?,?,?)',
                (order_id, item['item_id'], item['quantity'], item['unit_price'])
            )

        # Step 3: Validate + consume promo code if provided
        if data.get('promo_code'):
            ...
            if promo['used_count'] >= promo['max_uses']:
                raise ValueError(f"Promo code '{data['promo_code']}' has reached its usage limit")
            db.execute('UPDATE promo_codes SET used_count=used_count+1 WHERE code=?', (data['promo_code'],))

        # Step 4: Loyalty — ensure row exists, award points, deduct redeemed
        ...
        if loyalty_discount > 0:
            ...
            if not lp or lp['total_points'] < pts:
                raise ValueError(f"Insufficient loyalty points (have {lp['total_points'] if lp else 0}, need {pts})")
            db.execute('UPDATE loyalty_points SET total_points=total_points-? WHERE customer_id=?', (pts, cid))
            db.execute('INSERT INTO loyalty_transactions (customer_id,points,type,description,order_id) VALUES (?,?,?,?,?)',
                       (cid, -pts, 'redeem', f'Redeemed for PKR {pts} discount', order_id))

        db.execute('COMMIT')
        log_audit(cid, 'PLACE_ORDER', f"Order #{order_id} PKR {total} promo={data.get('promo_code','none')} pts={points_earned} COMMITTED")
        return jsonify({'success': True, 'order_id': order_id, 'total': total, 'points_earned': points_earned})

    except ValueError as e:
        db.execute('ROLLBACK')
        log_audit(data.get('customer_id'), 'ORDER_ROLLBACK', f"Validation error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 400
    except sqlite3.Error as e:
        db.execute('ROLLBACK')
        log_audit(data.get('customer_id'), 'ORDER_ROLLBACK', f"DB error: {e}")
        return jsonify({'success': False, 'error': 'Database error — order rolled back'}), 500
    except Exception as e:
        db.execute('ROLLBACK')
        log_audit(data.get('customer_id'), 'ORDER_ROLLBACK', f"Unexpected error: {e}")
        return jsonify({'success': False, 'error': 'Unexpected error — order rolled back'}), 500
    finally:
        db.close()

@app.route('/api/orders/customer/<int:cid>',    methods=['GET'])
@app.route('/api/v1/orders/customer/<int:cid>', methods=['GET'])
def customer_orders(cid):
    db = get_db()
    orders = db.execute('''SELECT o.*, r.name as restaurant_name,
                           ru.id as rider_user_id, ru.name as rider_name
                           FROM orders o
                           JOIN restaurants r ON o.restaurant_id=r.id
                           LEFT JOIN riders ri ON o.rider_id=ri.id
                           LEFT JOIN users ru ON ri.user_id=ru.id
    ...
    produce = db.execute('''SELECT po.*, 'Fresh Produce Market' as restaurant_name,
                              ...
                              LEFT JOIN riders ri ON po.rider_id=ri.id
                              LEFT JOIN users ru ON ri.user_id=ru.id
                              WHERE po.customer_id=? ORDER BY po.created_at DESC''', (cid,)).fetchall()
    for p in produce:
        pd = dict(p)
...
def admin_add_restaurant():
    data = request.json
    db = get_db()
    cur = db.execute("INSERT INTO restaurants (name,cuisine,location,phone,rating,delivery_time,status,added_by) VALUES (?,?,?,?,?,?,'approved',?)",
                     (data['name'],data.get('cuisine',''),data.get('location',''),data.get('phone',''),
...
def admin_delete_restaurant(rid):
    db = get_db()
    db.execute('UPDATE restaurants SET is_open=0 WHERE id=?',(rid,))
    db.commit()
...
def get_order_detail(oid):
    ...
    o = db.execute('''
        SELECT o.*, r.name as restaurant_name, r.location as restaurant_location,
               u.name as rider_name, ri.vehicle_type as rider_vehicle, u.id as rider_user_id
        FROM orders o
        JOIN restaurants r ON o.restaurant_id = r.id
        LEFT JOIN riders ri ON o.rider_id = ri.id
...
def get_produce():
    ...
    db = get_db()
    if category:
        rows = db.execute('SELECT * FROM produce_items WHERE category=? AND is_available=1 ORDER BY name', (category,)).fetchall()
    else:
        rows = db.execute('SELECT * FROM produce_items WHERE is_available=1 ORDER BY category, name').fetchall()
    db.close()
    return jsonify([dict(r) for r in rows])
...
def place_produce_order():
    """Place a produce order — fully atomic via BEGIN/COMMIT/ROLLBACK.

    Transaction scenario 2 (Phase 2 requirement):
    Steps: INSERT produce_order header → INSERT produce_order_items (each validated) →
           validate+UPDATE promo → INSERT OR IGNORE loyalty_points →
           UPDATE loyalty + INSERT loyalty_transaction → COMMIT
    Rollback conditions: unavailable produce item, invalid promo, loyalty underflow, any DB error.
    """
    data = request.json
    if not data:
        return jsonify({'success': False, 'error': 'Request body required'}), 400
    ...
    try:
        ...
        # Step 1: INSERT produce order header
        cur = db.execute(
            'INSERT INTO produce_orders (customer_id,total_amount,delivery_fee,payment_method,'
            'delivery_address,special_instructions) VALUES (?,?,?,?,?,?)',
            (cid, total, delivery_fee,
             data.get('payment_method','Cash on Delivery'),
        ...
        # Step 2: INSERT produce items — validate availability
        for item in data.get('items', []):
            pi = db.execute(
                'SELECT id FROM produce_items WHERE id=? AND is_available=1',
                (item['produce_id'],)
            ).fetchone()
            if not pi:
                raise ValueError(f"Produce item {item['produce_id']} is not available")
            db.execute(
                'INSERT INTO produce_order_items (order_id,produce_id,quantity_kg,unit_price) VALUES (?,?,?,?)',
                (order_id, item['produce_id'], item['quantity_kg'], item['unit_price'])
            )

        # Step 3: Validate + consume promo code
        if data.get('promo_code'):
            promo = db.execute(
                'SELECT * FROM promo_codes WHERE code=? AND is_active=1',
                (data['promo_code'],)
        ...
        db.execute('COMMIT')
        log_audit(cid, 'PRODUCE_ORDER', f"Produce order #{order_id} PKR {total} pts={points_earned} COMMITTED")
        return jsonify({'success': True, 'order_id': order_id, 'total': total, 'points_earned': points_earned})

    except ValueError as e:
        db.execute('ROLLBACK')
        log_audit(data.get('customer_id'), 'PRODUCE_ORDER_ROLLBACK', f"Validation error: {e}")
        return jsonify({'success': False, 'error': str(e)}), 400
    except sqlite3.Error as e:
        db.execute('ROLLBACK')
        log_audit(data.get('customer_id'), 'PRODUCE_ORDER_ROLLBACK', f"DB error: {e}")
        return jsonify({'success': False, 'error': 'Database error — order rolled back'}), 500
    except Exception as e:
        db.execute('ROLLBACK')
...
def update_produce_order_status(oid):
    ...
    params = [data['status'], datetime.datetime.now().isoformat()]
    if 'rider_id' in data and data['rider_id']:
        update_fields = 'status=?,rider_id=?,updated_at=?'
        params = [data['status'], data['rider_id'], datetime.datetime.now().isoformat()]
    params.append(oid)
    # update_fields is built from a hardcoded whitelist — safe, no user input in field names
...
def get_produce_order_detail(oid):
    ...
    o = db.execute('''SELECT po.*, u.name as customer_name,
                      ru.name as rider_name, r.vehicle_type as rider_vehicle, ru.id as rider_user_id
                      FROM produce_orders po
...
@app.route('/api/v1/loyalty/<int:cid>', methods=['GET'])
def get_loyalty(cid):
    db = get_db()
